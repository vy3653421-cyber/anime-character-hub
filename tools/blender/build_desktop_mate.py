...
print('Desktop Mate skin bindings:', [(o.name, any(m.type == 'ARMATURE' and m.object == arm for m in o.modifiers), o.parent == arm, o.parent_type, len(o.vertex_groups)) for o in parts if o.type == 'MESH'])
print('Desktop Mate NLA tracks:', [(track.name, len(track.strips), track.mute, track.strips[0].blend_type if track.strips else None) for track in arm.animation_data.nla_tracks])
