...
import type { MemoryPersistence } from "./memory-store";
...
export class MemoryStore {
  ...
  save(content: string, tags: string[] = []): MemoryEntry {
    const entry: MemoryEntry = {
      id: crypto.randomUUID(), createdAt: new Date().toISOString(), content, tags: [...tags],
    };
  ...
  search(query: string): MemoryEntry[] {
    ...
    return this.list().map((entry, index) => {
      ...
      return { entry, index, overlap, exactPhrase: haystack.includes(needle) ? 1 : 0 };
    }).filter(({ overlap }) => overlap > 0)
      .sort((a, b) => b.exactPhrase - a.exactPhrase || b.overlap - a.overlap || b.index - a.index)
      .map(({ entry }) => entry);
  }
  ...
  private async load(): Promise<void> {
    ...
    for (const entry of await this.persistence.load()) this.entries.set(entry.id, entry);
  }

  private scheduleFlush(): void {
    if (!this.persistence) return;
    this.flushPromise = this.flushPromise.then(() => this.persistence!.save(this.list()));
  }
}
