...
export function ArchiveControls({ query, onQueryChange, filter, onFilterChange, sort, onSortChange, savedCount }: Props) {
  ...
  return (
    <div className="space-y-3 lg:sticky lg:top-4 lg:z-30">
        ...
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
          <div className="group relative w-full min-w-0 lg:max-w-2xl lg:flex-1">
            ...
            <input
              ...
              autoComplete="off"
              spellCheck={false}
              className="w-full rounded-2xl border border-transparent bg-white/[.035] py-3.5 pl-11 pr-24 text-sm outline-none transition-[background-color,border-color,box-shadow] duration-300 placeholder:text-white/30 focus:border-violet-300/35 focus:bg-white/[.065] focus:shadow-[0_0_0_4px_rgba(167,139,250,.06)]"
            />
            <div className="pointer-events-none absolute right-3 top-1/2 hidden -translate-y-1/2 items-center gap-1 rounded-lg border border-white/8 bg-white/[.04] px-2 py-1 text-[9px] font-medium text-white/25 sm:flex">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <label className="flex items-center gap-2 rounded-2xl border border-white/8 bg-white/[.025] px-3.5 py-2.5 text-xs text-white/55 transition-colors duration-300 hover:border-white/15 hover:text-white/75">
      ...
      <div className="flex min-h-4 items-center justify-between px-1 text-[10px] uppercase tracking-[.18em] text-white/25">
        <span>{query ? `Searching for “${query}”` : filter === "saved" ? "Saved collection" : "Browse the archive"}</span>
        <span className="hidden items-center gap-2 sm:flex">
          {hasActiveView && <span className="h-1 w-1 rounded-full bg-violet-300/70" aria-hidden="true" />}
