...
const RECENT_SEARCHES_KEY = "anime-character-hub:recent-searches";
const RECENT_LIMIT = 6;

function readRecentSearches(): string[] {
  try {
    const value = JSON.parse(window.localStorage.getItem(RECENT_SEARCHES_KEY) ?? "[]") as unknown;
...
export function ArchiveControls({ query, onQueryChange, filter, onFilterChange, sort, onSortChange, savedCount, onReset }: Props) {
  ...
  const rememberSearch = () => {
    ...
    if (!value) return;
    const next = [value, ...readRecentSearches().filter((item) => item.toLowerCase() !== value.toLowerCase())].slice(0, RECENT_LIMIT);
    try { window.localStorage.setItem(RECENT_SEARCHES_KEY, JSON.stringify(next)); } catch { /* Optional enhancement. */ }
    setRecentSearches(next);
  };

  const clearRecentSearches = () => {
    try { window.localStorage.removeItem(RECENT_SEARCHES_KEY); } catch { /* Optional enhancement. */ }
    setRecentSearches([]);
  };
      ...
      <div className="rounded-3xl border border-white/8 bg-[#0a0a0f]/75 p-2 shadow-[0_18px_60px_rgba(0,0,0,.18)] backdrop-blur-xl">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
          <div className="group relative w-full min-w-0 lg:max-w-2xl lg:flex-1">
            ...
            {query && <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => onQueryChange("")} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-white/[.06] p-1.5 text-white/40 transition duration-300 hover:scale-105 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/80" aria-label="Clear search"><X size={15} aria-hidden="true" /></button>}
            {focused && !query && recentSearches.length > 0 && (
              <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-50 rounded-2xl border border-white/10 bg-[#0d0c13]/98 p-3 shadow-2xl shadow-black/50 backdrop-blur-2xl" onMouseDown={(event) => event.preventDefault()}>
                <div className="mb-2 flex items-center justify-between px-1"><span className="text-[9px] uppercase tracking-[.2em] text-white/30">Recent searches</span><button type="button" onClick={clearRecentSearches} className="text-[9px] font-semibold text-white/30 transition hover:text-white/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70">Clear</button></div>
                <div className="flex flex-wrap gap-2">{recentSearches.map((item) => <button key={item} type="button" onClick={() => { onQueryChange(item); setFocused(false); }} className="max-w-full truncate rounded-full border border-white/8 bg-white/[.04] px-3 py-1.5 text-[10px] font-medium text-white/55 transition hover:border-violet-300/25 hover:bg-violet-300/[.06] hover:text-violet-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70">{item}</button>)}</div>
              </div>
            )}
          </div>
