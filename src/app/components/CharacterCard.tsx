  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-white/8 bg-white/[.035] transition duration-500 hover:-translate-y-1 hover:border-white/15">
      ...
      <div className="aspect-[4/5] overflow-hidden">
        {character.image ? (
          <img src={character.image} alt={character.name} loading={rank > 4 ? "lazy" : "eager"} className="h-full w-full object-cover opacity-70 transition duration-700 group-hover:scale-105 group-hover:opacity-95" />
        ) : (
      ...
      <button onClick={() => onToggleSaved(character.id)} className="absolute right-4 top-4 z-20 rounded-full border border-white/10 bg-black/35 p-2 text-white/70 backdrop-blur-md transition hover:bg-black/60 hover:text-white" aria-label={`${saved ? "Remove" : "Save"} ${character.name}`}>
