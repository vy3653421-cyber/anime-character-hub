          ...
          <div id="popular" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {filteredCharacters.map((character, index) => (
              <article key={character.id} className="group relative overflow-hidden rounded-3xl border border-white/8 bg-white/[.035] transition duration-500 hover:-translate-y-1 hover:border-white/15">
                ...
                <button onClick={() => toggleFavorite(character.id)} className="absolute right-4 top-4 rounded-full border border-white/10 bg-black/35 p-2 text-white/70 backdrop-blur-md transition hover:bg-black/60 hover:text-white" aria-label={`Save ${character.name}`}><Star size={14} fill={favorites.includes(character.id) ? "currentColor" : "none"} /></button>
                <div className="absolute bottom-0 left-0 right-0 p-5"><div className="mb-2 text-[9px] uppercase tracking-[.2em] text-violet-300">Rank {index + 1}</div><h3 className="text-lg font-bold">{character.name}</h3><p className="mt-1 text-xs text-white/40">{character.favorites.toLocaleString()} community favorites</p></div>
              </article>
            ))}
          </div>
